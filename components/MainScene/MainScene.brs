...
sub init()
    ...
    ' Establece la imagen de fondo de la pantalla principal
    ...
    m.get_channel_list = m.top.FindNode("get_channel_list") 'Obtiene la URL del registro y analiza el feed
    m.get_channel_list.ObserveField("content", "SetContent") '¿Se ha analizado el contenido? Si es así, va a la subrutina SetContent y muestra la lista
    m.list = m.top.FindNode("list")
    m.list.ObserveField("itemSelected", "setChannel") 
    m.video = m.top.FindNode("Video")
    m.video.ObserveField("state", "checkState")

    ' Muestra la lista directamente sin mostrar el diálogo de teclado
...
sub showdialog()
    ...
    keyboarddialog.backgroundUri = "pkg:/images/fondo_url-2.jpg"
    keyboarddialog.title = "Ingresar URL"

    keyboarddialog.buttons=["OK","Ver Demo", "Guardar"]
